 ...
 * @author RUCHI
 */
public class VoterShow {
     Winner win = new Winner();
    public  String display(String id,String name,String Phone,String email,boolean flag,int volterlist,String[] canname,String[] canid,String candidentname,int candidentid)
    {
...
output+="    </table>\n" ;
        ...
        {
                  output+="<h4 style=\"text-align:center; padding-top: 10px; padding-bottom: 10px; \">Available Candidate </h4>\n" +
                    "<hr>\n" +
                   "<form action=\"voting\" method=\"post\">\n" +
                   "<input name=\"voterid\" hidden=\"\" value=\""+id+"\">\n" +
                   "<div class=\"form-group row\" style=\"text-align: left;  max-width: 300px\">\n";
                   for(int r=0;r<volterlist;r++){
                  output+="<h4 id=\"votername\">"+canname[r]+"</h4> \n" +
                   "<input type=\"radio\" id=\"votertext\" name=\"voteid\"  value=\""+canid[r]+"\" required >\n"
                          + "<hr>" ;
                   }
                   output+="<button type=\"submit\"  class=\"btn-primary\" style=\"width: 150px; margin-left: 80px; margin-top: 20px; height: 50px; border-radius: 7px\">Vote</button>\n" +
                           "</div>\n" +
                         "</form>\n" ;
        }
       else if(candidentid==0)
       {
        output+="<div style=\"width:100%; text-align:center\"><h4 style=\"padding-top:30px\"> You Have Choose NOTA</h4></div>";
       }
       else
       {
        output+="<div style=\"width:100%; text-align:center\"><h4 style=\"padding-top:30px\"> Already voted  : "+candidentname+"</h4></div>";
       }
                  output+="</div>\n" +
"               </div>\n" +
"    </body>\n" +
